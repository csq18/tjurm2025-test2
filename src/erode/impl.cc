...
std::vector<cv::Mat> erode(const cv::Mat& src_erode, const cv::Mat& src_dilate) {
    ...
    cv::cvtColor(src_erode,gray_dilate,cv::COLOR_BGR2GRAY);
    cv::cvtColor(src_dilate,gray_dilate,cv::COLOR_BGR2GRAY);

    cv::Mat thresh_erode,thresh_dilate;
    cv::threshold(gray_erode,thresh_erode,50,255,cv::THRESH_BINARY);
    cv::threshold(gray_dilate,thresh_dilate,50,255,cv::THRESH_BINARY);
    
    cv::Mat kernel = cv::getStructuringElement(cv::BORDER_CONSTANT,cv::Size(3,3));
    cv::erode(thresh_erode,dst_erode,kernel,cv::Point(-1,-1),1,cv::BORDER_CONSTANT,cv::morphologyDefaultBorderValue());
    cv::dilate(thresh_dilate,dst_dilate,kernel,cv::Point(-1,-1),1,cv::BORDER_CONSTANT,cv::morphologyDefaultBorderValue());
